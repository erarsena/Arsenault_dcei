...
Results from six Layman's metrics suggest differences in food web structure among sites of different hydrogeomorphology (Figure 2). Maximum N range (`r laydf$MMR$metrics["dY_range"]`) and maximum C range (`r laydf$MMR$metrics["dX_range"]`) both occurred at site MMR. The largest total area (`r laydf$WIN$metrics["TA"]`) and the maximum centroid distance among the sites (`r laydf$WIN$metrics["CD"]`) were both observed at site WIN. Maximum mean nearest neighbor distance occurred at site PEP (`r laydf$PEP$metrics["NND"]`), while the highest standard deviation of the nearest neighbor distance (`r laydf$MMR$metrics["SDNND"]`) was observed at site MMR (Table 1).
...
```{r layman, include=T, echo=F, warning=F, fig.cap="Layman's metrics for four sites along the Upper Mississippi River (SCR, PEP, WIN, and MMR). Layman's metrics include d15N range (dY_range), d13C range (dX_range), total area (TA), centroid distance (CD), mean nearest neighbor distance (NND), and standard deviation of the nearest neighbor distance (SDNND)."}

#Dependencies: layman_calc() function, data generated from tp_calc function

knitr::kable(as.matrix(layman_calc(tpdf)))
...
Recent expansions to river conceptual models suggest that rivers can be more accurately viewed as patches of varying hydrogeomorphic complexity, where river segments with related hydrogeomorphologic characteristics are expected to be more similar than river segments related by their geographic proximity alone. Here, our results suggest that hydrogeomorphic complexity may be a possible predictor of certain aspects of food web structure.

Hydrogeomorphic complexity values varied among UMR sites. As expected, the greatest hydrogeomorphic complexity was observed at site WIN, a naturally anastomosing site with many channels and islands. The second highest complexity occurred at site PEP; however, the complexity value at this site may have been inflated due to its very high wetted width value, since PEP is an artificial reservoir (Lake Pepin). In future studies, it may be necessary to account for the effect of reservoirs in a different way. MMR, the site furthest downstream on the main channel of the UMR had the next highest complexity value, and SCR, a site in the headwaters of the UMR, had the lowest complexity score overall. These values were also as expected. Both MMR and SCR were largely single-channel areas with few islands and side channels, but the MMR site downstream had a larger wetted width value than the smaller river segments in the St. Croix headwaters.

Food chain length was highest at SCR, the site with the lowest hydrogeomorphic complexity, which does not support our hypothesis that food chain length would increase with habitat complexity. The individual that was observed to be at the top of the food web at SCR was a largemouth bass (___), which are known to be aggressive predators. It is possible that food chain length might have been higher than expected in these headwater areas because that section of the UMR is partially protected by a state park ____. Higher forest cover and natural habitat in the surrounding riparian areas might allow large fish more opportunities to feed on terrestrial subsidies such as small mammals that might fall into the river and drown. The shortest food chain length was observed in PEP, at Lake Pepin. While lakes tend to have long food chains, this may have been because of the artificial nature of the reservoir. More work should be conducted to better understand how anthropogenic disturbances may be disrupting the natural aquatic food web in this area.

Different aspects of food web complexity, as quantified using Layman's metrics, suggested different patterns when compared to site complexity scores. MMR had the widest spread of both $\delta^{13}C$ and $\delta^{15}N$ values, which suggests a food web supported by a large diversity of different food sources, which supports many different trophic roles, from top predators down to detritivores. Total area (TA) is a measure of total niche space available at a site. Site WIN had the highest TA, which may suggest that hydrogeomorphic complexity does support complex ecological roles in the environment, even if those differences in trophic roles do not ultimately support the highest food chain length along the UMR. 

The next three Layman's metrics uncover food web dynamics happening at a more specific, individual level. 
